...
@pytest.mark.parametrize("pred_reqs_info", [[(12, 3),(15, 4),(15, 2)]])
@pytest.mark.parametrize("allocator_type", ["naive"])
def test_len_aware_num_required_blocks(block_size: int, 
                                       pred_reqs_info: List[Tuple[int, int]], 
                                       allocator_type: str):
    """Verify correct calculation of num_required_blocks when using len-aware
    allocation. This is done by allocating a sequence and then appending tokens
    to it. The number of consumed blocks should match the expected number of
    blocks.
    """
    num_gpu_blocks = 1024
    ...
    require_blocks = block_table.len_aware_get_num_required_blocks(pred_reqs_info, block_size)
    if block_size == 1:
        assert require_blocks == 48
    elif block_size == 8:
        assert require_blocks == 8
    elif block_size == 16:
        assert require_blocks == 5