...
def initialize_scheduler(
    *,
    ...
    decode_prediction_model_path="analytic_model/config/llama8b-sharegpt-decode.json",
    prefill_prediction_model_path="analytic_model/config/llama8b-sharegpt-prefill.json",
    scheduling_policy="ldf_rej_ada1.15_pred"
    ...
    scheduler_config = SchedulerConfig(
        ...
        decode_prediction_model_path=decode_prediction_model_path,
        prefill_prediction_model_path=prefill_prediction_model_path,
...
def test_delay_reorder():
    scheduler = initialize_scheduler(
        ...
        max_num_seqs=1000,
        max_model_len=1000,
        policy="ldf_rej_ada1.15_pred"
    )
    seqB, seqB_group = create_dummy_prompt("1",
                                           prompt_tokens=list(range(8)),
                                           block_size=4)
    seqB_group.ttft = 10000
    seqC, seqC_group = create_dummy_prompt("2",
                                           prompt_tokens=list(range(8)),
                                           block_size=4)
    seqC_group.ttft = 20000
    seqD, seqD_group = create_dummy_prompt("3",
                                           prompt_tokens=list(range(8)),
                                           block_size=4)
    seqD_group.ttft = 30000
    seqE, seqE_group = create_dummy_prompt("4",
                                           prompt_tokens=list(range(8)),
                                           block_size=4)
    seqE_group.ttft = 40000
    seqF, seqF_group = create_dummy_prompt("5",
                                           prompt_tokens=list(range(8)),
                                           block_size=4)
    seqF_group.ttft = 50000
    seqG, seqG_group = create_dummy_prompt("6",
                                           prompt_tokens=list(range(8)),
                                           block_size=4)
    seqG_group.ttft = 60000
    
    scheduler.add_seq_group(seqB_group)
    scheduler.add_seq_group(seqC_group)
    scheduler.add_seq_group(seqD_group)
    scheduler.add_seq_group(seqE_group)
    scheduler.add_seq_group(seqF_group)
    scheduler.add_seq_group(seqG_group)
    
    # without delay
    ordered_requests = list(scheduler._get_ldf_ordered_requests())
    assert ordered_requests == [seqB_group, seqC_group, seqD_group, seqE_group, seqF_group, seqG_group]

    # with delay
    seqB_group.delay = True
    seqB_group.priority -= 1
    seqC_group.delay = True
    seqC_group.priority -= 1
    seqD_group.delay = True
    seqD_group.priority -= 1
    ordered_requests = list(scheduler._get_ldf_ordered_requests())
    assert ordered_requests == [seqE_group, seqF_group, seqG_group, seqB_group, seqC_group, seqD_group]

@pytest.mark.finegrained_admission_control
def test_finegrained_admission_control():
    scheduler = initialize_scheduler(
        block_size=4,
        num_cpu_blocks=16,
        num_gpu_blocks=16,
    ...
    seq1, seq1_group = create_dummy_prompt("1",
                                           prompt_tokens=list(range(8)),
                                           block_size=4)
    seq1_group.tpot = 30
    seq1_group.ttft = 1000
    seq2, seq2_group = create_dummy_prompt("2",
                                           prompt_tokens=list(range(8)),
                                           block_size=4)
    seq2_group.tpot = 30
    seq2_group.ttft = 1000
    seq3, seq3_group = create_dummy_prompt("3",
                                           prompt_tokens=list(range(8)),
                                           block_size=4)
    seq3_group.tpot = 60
    seq3_group.ttft = 1000
    seq4, seq4_group = create_dummy_prompt("4",
                                           prompt_tokens=list(range(8)),
                                           block_size=4)
    seq4_group.tpot = 60
    seq4_group.ttft = 1000
    ...
    # check tpot state manager
    seq_group_metadata_list, scheduler_outputs, _ = scheduler.credit_based_schedule()
    assert scheduler.tpot_state.get_lowest_tpot() == 30
    assert scheduler.tpot_state.get_virtual_batch_size() == 30/30 + 30/30 + 30/60 + 30/60

    # first iter, only seq1 and seq2 can be scheduled
    seq_group_metadata_list, scheduler_outputs, _ = scheduler.credit_based_schedule()
    assert len(scheduler_outputs.scheduled_seq_groups) == 2
    assert set(scheduler_outputs.scheduled_seq_groups) == {seq1_group, seq2_group}
    
    # seq3, seq4 will be scheduled in the next iter
    seq_group_metadata_list, scheduler_outputs, _ = scheduler.credit_based_schedule()
    assert len(scheduler_outputs.scheduled_seq_groups) == 4
    assert set(scheduler_outputs.scheduled_seq_groups) == {seq1_group, seq2_group, seq3_group, seq4_group}