...
from vllm.engine.multiprocessing import (ENGINE_DEAD_ERROR, IPC_DATA_EXT,
                                         ...
                                         RPCUProfileRequest, RPCSchedulerStatsRequest)
# yapf: enable
...
class MQLLMEngine:
            ...
            while self.input_socket.poll(timeout=0) != 0:
                ...
                if isinstance(request, RPCProcessRequest):
                    ...
                    if request == RPCUProfileRequest.START_PROFILE:
                        ...
                        self.stop_profile()
                elif isinstance(request, RPCSchedulerStatsRequest):
                    self._handle_scheduler_stats_request(request)
                else:
    ...
    def _handle_scheduler_stats_request(self, request: RPCSchedulerStatsRequest):
        """Handle request for scheduler statistics"""
        try:
            request_id = request.request_id
            # Collect scheduler statistics
            stats = {}            
            scheduler = self.engine.scheduler[0] if isinstance(self.engine.scheduler, list) else self.engine.scheduler
            
            # Collect the metrics you're interested in
            stats["scheduler_profile"] = scheduler.get_scheduler_profile()
            scheduler.reset_schedule_profile()
            scheduler.set_next_schedule_policy()
            # Send the stats via the output socket
            stats_output = {"request_id": request_id, "stats": stats, "finished": True}
            self._send_outputs([stats_output])
        except Exception as e:
            self._set_errored(e)
            self._send_outputs({"error": str(e)})
