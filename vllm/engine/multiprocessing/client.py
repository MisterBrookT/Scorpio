...
class MQLLMEngineClient(EngineClient):
    ...
    async def run_output_handler_loop(self):
        ...
        try:
            while True:
                ...
                elif isinstance(request_outputs, list) and isinstance(request_outputs[0], dict) and "request_id" in request_outputs[0] and "stats" in request_outputs[0]:
                    # Extract the stats output from the list
                    stats_output = request_outputs[0]
                    request_id = stats_output["request_id"]
                    stats = stats_output["stats"]
    ...
    async def get_scheduler_stats(self) -> Dict[str, Any]:
        """Get scheduler statistics from the engine."""
        try:
            import uuid
            # Create a unique request ID that will be used on both client and server
            stats_request_id = f"scheduler_stats_{uuid.uuid4()}"
            
            # Create a temporary queue for this request
            queue: asyncio.Queue = asyncio.Queue()
            self.output_queues[stats_request_id] = queue
            
            # Send request to the engine with the request_id
            request = RPCSchedulerStatsRequest(request_id=stats_request_id)
            await self.input_socket.send_multipart((pickle.dumps(request),), copy=False)
            
            try:
                # Wait for response with timeout
                try:
                    response = await asyncio.wait_for(queue.get(), timeout=VLLM_RPC_TIMEOUT/1000)
                    if isinstance(response, BaseException):
                        raise response
                    return response
                except asyncio.TimeoutError:
                    raise TimeoutError("Timed out waiting for scheduler stats")
            finally:
                # Clean up the temporary queue
                self.output_queues.pop(stats_request_id, None)
