...
class EngineArgs:
    ...
    model: str = '/root/autodl-pub/models/opt-125'
    ...
    collect_decode_time_file: Optional[str] = None
    collect_prefill_time_file: Optional[str] = None
    ...
    # AdaPlace configuration
    prefill_prediction_model_path: str = ""
    decode_prediction_model_path: str = ""
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        parser.add_argument('--collect-decode-time-file',
                            type=str,
                            help='Collect decode time metrics for profiling.')
        parser.add_argument('--collect-prefill-time-file',
                            type=str,
                            help='Collect prefill time metrics for profiling.')
        ...
        parser.add_argument(
            '--scheduling-policy',
            nargs='*', # '*' means 0 or more arguments will be collected into a list
                    # '+' means 1 or more arguments will be collected into a list
            default=["fcfs"],
            help='The scheduling policy(ies) to use. Possible values include "fcfs", "priority". Defaults to ["fcfs"].'
        )


        parser.add_argument(
            '--prefill-prediction-model-path',
            type=str,
            default="",
            help='Path to the model used for predicting prefill latency in AdaPlace adaptive placement.')

        parser.add_argument(
            '--decode-prediction-model-path',
            type=str,
            default="",
            help='Path to the model used for predicting decoding latency in AdaPlace adaptive placement.')
        ...
        parser.add_argument(
            '--ada-place',
            action='store_true',
            help='Enable AdaPlace for the model. '
        )
        parser.add_argument(
            '--slo-tpot',
            type=float,
    ...
    def create_engine_config(self,
        ...
        scheduler_config = SchedulerConfig(
            ...
            prefill_prediction_model_path=self.prefill_prediction_model_path,
            decode_prediction_model_path=self.decode_prediction_model_path)
        ...
        profiling_config = ProfilingConfig(
            collect_decode_time_file=self.collect_decode_time_file,
            collect_prefill_time_file=self.collect_prefill_time_file,
        )
